...
const CreateNew = (props) => {
  ...
  const newInfo = useField('info')

  let listAll = [newContent, newAuthor, newInfo]

const content = newContent.value
const author = newAuthor.value
const info = newInfo.value

const listTwo = listAll.map(({ resetAll, ...newObj }) => newObj)
const secondContent = listTwo.find(x=>x.name==='content')
const secondAuthor = listTwo.find(x=>x.name==='author')
const secondInfo = listTwo.find(x=>x.name==='info')

const handleReset = (e)=>{
  e.preventDefault()
listAll.map(x=>x.resetAll())
}
